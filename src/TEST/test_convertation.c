...
START_TEST(int_to_dec_1) {
  ...
  int result_error = 0;
  int my_error = d3_from_int_to_decimal(src, &result);
...
START_TEST(dec_to_int_1) {
  ...
  int origin = 5;
  int origin_error = 0;
  int my_error = d3_from_decimal_to_int(src, &result);
...
START_TEST(dec_to_float_1) {
  ...
  int result_error = 0;
  float my_float;
  int my_error = d3_from_decimal_to_float(number, &my_float);
...
START_TEST(float_to_dec__1) {
  float num = 829834.3;
  d3_decimal res_bits = {{0, 0, 0, 0}};

  int res = d3_from_float_to_decimal(num, &res_bits);
...
Suite *test_convertation() {
  Suite *s = suite_create("d3_convertation");
  TCase *o = tcase_create("d3_convertation");
  suite_add_tcase(s, o);
